...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (self.titleString.length > 0) {
        ...
        NSString *str_add = [[NSUserDefaults standardUserDefaults]valueForKey:@"add"];
        
        if([str_add isEqualToString:@"requirement"])
            _mainEditorView.topViewLabel.text = @"Add Requirement";
        else if([str_add isEqualToString:@"message"])
            _mainEditorView.topViewLabel.text = @"Message";
        else
            _mainEditorView.topViewLabel.text = @"Add task";
    }
