...
- (void)dismissPopup:(PopupButtonType)buttonType {
    ...
    else
    {
        ...
        {
            UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"" message:@"Project Name cannot be empty!" delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil, nil];
            [alert show];
        }
        else
        {
            [self configureOutgoingAnimationFor:outgoingTransitionType ? outgoingTransitionType : PopupOutgoingTransitionTypeDisappearCenter withButtonType:buttonType];
            [[NSUserDefaults standardUserDefaults]setValue:textField1.text forKey:@"projectName"];
        }
    }
